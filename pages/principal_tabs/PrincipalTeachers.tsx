
import React from 'react';
import { GlassCard } from '../../components/GlassUI';
import { UserCheck } from 'lucide-react';

const PrincipalTeachers: React.FC = () => {
    return (
        <div className="animate-fade-in-up">
            <GlassCard className="flex flex-col items-center justify-center min-h-[400px] text-center">
                <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-4 text-blue-600">
                    <UserCheck className="w-8 h-8"/>
                </div>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white">Teacher Monitoring</h3>
                <p className="text-slate-500 mt-2 max-w-sm">
                    Track mark entry status, teacher activity, and class performance here.
                </p>
                <div className="mt-6 px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-xs font-mono text-slate-500">
                    Module: TEACHER_OVERSIGHT
                </div>
            </GlassCard>
        </div>
    );
};

export default PrincipalTeachers;
